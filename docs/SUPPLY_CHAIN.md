# Supply-chain and secret-management procedure

This repository treats every executable dependency and container base as an
immutable build input. A tag or a package name without a digest, exact version,
or file hash is not sufficient evidence for production.

## Authoritative inputs

The CV assistant image is built from these committed inputs:

- `bot/Dockerfile` with a digest-pinned Python base image;
- `bot/requirements.in` with the three reviewed direct dependencies;
- `bot/requirements.txt` generated by `pip-compile --generate-hashes`;
- `bot/app.py` and `bot/storage.py`;
- the least-privilege `docker-compose.yml` contract;
- `security/supply-chain.json`, which records image identities and the audited
  resolved Python graph.

`scripts/build-input-id.py` hashes the path and complete contents of every input.
The resulting SHA-256 value and exact Git commit are written into OCI labels on
the built `cvbot` image. Hosted CI verifies those labels on its local build. The
RPi5 production helper independently rebuilds the ARM64 image from the same
immutable inputs and rejects it unless the revision and build-input labels match
the exact source SHA being deployed.

## Updating Python packages

1. Create an isolated issue and branch.
2. Review the release notes and security advisories for each direct dependency.
3. Change only exact versions in `bot/requirements.in`.
4. Create a Python 3.12 resolver venv and pin the resolver itself to
   `pip==26.0.1` and `pip-tools==7.5.3`. Regenerate `bot/requirements.txt`
   with `--generate-hashes --resolver=backtracking --strip-extras`.
5. Verify the lock with `pip install --dry-run --require-hashes --no-deps`.
6. Query the exact PyPI release metadata for every resolved package. Reject a
   release when `info.yanked` is true or `vulnerabilities` is non-empty.
7. Run the complete behavior suite, hosted image build/provenance check and image
   vulnerability scan before merge. The production ARM64 rebuild is a separate
   post-merge gate and must prove the same immutable build identity before start.
8. Commit both the human-reviewed direct requirements and generated hash-complete
   `bot/requirements.txt` in the same pull request.

Never edit individual hashes or transitive versions by hand. Do not let the
hosted runner's preinstalled `pip` version implicitly select the resolver
behavior; the resolver toolchain is part of the reproducibility boundary.

## Updating container images

1. Resolve the multi-platform manifest digest from the image's official registry.
2. Record the tag and digest in `security/supply-chain.json`.
3. Replace the matching digest in the Dockerfile or Compose file.
4. Build in hosted CI, inspect OCI labels, and run Trivy against that build. On
   production, rebuild on ARM64 from the same digest-pinned inputs and verify the
   exact revision/build-input labels before container start.
5. Reject unresolved HIGH or CRITICAL findings. An exception requires a separate
   issue with package, advisory, applicability, compensating control, owner, and
   expiry date; it must not be hidden in a global ignore file.

Shared Cloudflare ingress is host-wide infrastructure owned by `RPi5_main` and
runs on the RPi5 as systemd `cloudflared.service`. This repository does not pin,
install, deploy, restart, reconcile or roll back that connector. CV deployment
may verify `https://rozkalns.net/`, but shared ingress lifecycle and credentials
remain outside the CV application ownership boundary.

## Secret scanning

`scripts/run-gitleaks.sh` uses an immutable Gitleaks 8.30.0 container image and
runs three fail-closed layers:

1. an independent synthetic GitHub-token canary must be detected with exit code 1;
2. nine runtime-generated fixture classes must each be detected: Cloudflare
   tunnel token, Cloudflare API credential, AWS credential, private key, webhook
   secret, JWT, password assignment, database URL password, and generic
   high-entropy API key;
3. the complete repository history is scanned with `git --log-opts=--all` and
   all reported secrets are redacted.

All fixture credential values are assembled only at runtime from deterministic
fragments or hashes; complete fixture secrets are not stored in Git source or
history. The canary and matrix protect against a scanner that starts successfully
but silently stops detecting supported credential classes. Gitleaks 8.30.1 is
intentionally prohibited because its release has a confirmed detection regression.

Temporary secret-test implementation commits must not become permanent baseline
entries merely to make a pull request green. Before merge, squash/rebuild the PR
branch onto its exact base when necessary so transient fixture-generator source
is not preserved as reachable branch history; the final tree must then pass the
same full-history scan again.

`.gitleaks.toml` extends the upstream default rules and adds narrow project rules
for Cloudflare tunnel/API credentials, database URL passwords, webhook/signing
secrets, and direct password assignments. Allowlists contain only explicit
test/example placeholders. Do not add a commit, file, or broad regular expression
to suppress a real finding.

### Exact historical baseline

`.gitleaksignore` contains exactly three historical fingerprints. All three are
the same Cloudflare Web Analytics browser beacon site token in `html/index.html`,
reported by the generic API-key heuristic. Cloudflare's own Web Analytics setup
places this site token directly in the public client-side `data-cf-beacon`
script snippet, so it is not an API key, tunnel credential, or other secret.

The baseline is intentionally fingerprint-specific: it covers only those three
commit/file/rule/line findings and does not exempt `html/index.html`, the
`generic-api-key` rule, Cloudflare strings, or future findings. Any change in
location, rule, or commit must be reviewed as a new finding.

Baseline review date: **2027-02-07**, or earlier whenever the Web Analytics
snippet/token is changed, Gitleaks is upgraded, or Cloudflare changes the beacon
credential model. If the token ever gains authentication capability, remove the
baseline and treat every historical occurrence as exposed.

## Responding to a finding

A secret found in any commit is considered exposed even when a later commit
removed it.

1. Stop the merge or deploy.
2. Identify the affected provider and scope without printing the secret.
3. Revoke or rotate the credential first.
4. Update the host-only secret store on the RPi5 with restrictive permissions.
5. Restart only the affected service and verify health.
6. Decide whether Git history must be rewritten. Coordinate history rewrites
   explicitly because all clones and open branches are affected.
7. Re-run the canary, fixture matrix, and full-history scan.
8. Record the provider, rotation date, affected service, verification evidence,
   and old credential revocation in a private incident record. Do not commit the
   credential or a reversible fingerprint.

The historical Cloudflare tunnel token mentioned in project migration notes must
be treated as exposed until independent evidence confirms that it was rotated.
The repository cannot prove rotation by itself; this is a host/provider action.

## Runtime boundary

`cvbot` runs as UID/GID 10001 with a read-only root filesystem, no Linux
capabilities, `no-new-privileges`, a PID limit, and a small no-exec `/tmp` tmpfs.
Only `bot/data` is writable. The deployment helper verifies that this directory
is not a symlink, migrates its ownership and private permissions before start,
and preserves it outside managed source sync and backups.

Rollback remains compatible because the older root-running image can read the
same files if a newly deployed least-privilege image fails before transaction
commit.

The pull-controller preflight independently resolves current `origin/main`,
requires successful exact-SHA CI and classifies the complete
production-to-target range. For an `AUTO_DEPLOY_SAFE` range, it also requires
the installed root-owned deploy-library and pull wrapper blobs to match their
exact source blobs in the target SHA before reporting the transport ready. The
preflight itself never authorizes production mutation; manual, database, host,
control-plane and no-deploy outcomes remain non-automatic and fail closed.

## Release evidence

A supply-chain-changing pull request is ready only when it contains:

- exact image and package identities;
- a hash-complete `bot/requirements.txt`;
- PyPI vulnerability/yank audit output;
- Gitleaks canary, nine-class fixture matrix, and full-history PASS;
- source validation and behavior tests;
- hosted image build with matching revision/build-input labels;
- Trivy HIGH/CRITICAL PASS on the hosted build;
- Compose rendering and nginx validation;
- the fail-closed pull-controller preflight and transport identity contract;
- no temporary resolver/finalizer workflow remaining.

Production release is complete only after the RPi5 rebuilds on ARM64 from the
same immutable inputs, verifies the image identity before start, passes service
health/public HTTP checks, and preserves the deploy evidence.
