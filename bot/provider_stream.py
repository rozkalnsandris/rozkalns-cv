...
class ProviderStreamParser:
    """Small fail-closed parser for DeepSeek data-only SSE chat streams."""
    ...
    def _parse_usage(self, payload: Any) -> ProviderUsage:
        ...
        return ProviderUsage(
            prompt_tokens=self._optional_nonnegative_int(
                payload.get("prompt_tokens"), "prompt_tokens"
            ),
            completion_tokens=self._optional_nonnegative_int(
                payload.get("completion_tokens"), "completion_tokens"
            ),
            total_tokens=self._optional_nonnegative_int(
    ...
    def feed_line(self, line: str) -> list[ProviderStreamEvent]:
        ...
        if raw == "[DONE]":
            if self.finish_reason is None:
                raise ProviderStreamError("[DONE] received before terminal finish_reason")
            self.done = True
        ...
        if finish_reason is not None:
            ...
            if self.finish_reason is not None:
                raise ProviderStreamError("duplicate terminal finish_reason")
